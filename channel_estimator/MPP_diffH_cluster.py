...
def train(size, hidden_sizes=[64, 32], lr=1e-3, channel_information = [0, 1, 0, 0.1],
    ...
    plt.title(' $[n_R,n_T,T]$:[%s,%s,%s], lr:%s, size:%s, $\\mu_H$:%s' 
                 %(n_R,n_T,T,lr, [2*n_R*T]+hidden_sizes+[2*2*n_R*n_T], H_mean))
    plt.xlabel('epochs')
    plt.ylabel('NMSE')
    plt.legend()
    plt.grid(True)
    plt.savefig('./simulation/result/N_lr%s_%s_ep%s_mu%s.png' %(lr, [2*n_R*T]+hidden_sizes+[2*2*n_R*n_T],num_epochs,H_mean))
    plt.close()
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig('./simulation/result/M_lr%s_%s_ep%s_mu%s.png' %(lr, [2*n_R*T]+hidden_sizes+[2*2*n_R*n_T],num_epochs,H_mean))

    ## save the model
    torch.save(logits_net, './simulation/result/lr%s_%s_ep%s_mu%s.pt' %(lr, [2*n_R*T]+hidden_sizes+[2*2*n_R*n_T],num_epochs,H_mean))
