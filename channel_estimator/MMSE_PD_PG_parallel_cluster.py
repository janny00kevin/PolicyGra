...
def train(size, hidden_sizes=[64, 32], lr=1e-3, channel_information = [0, 1, 0, 0.1],
    ...
    plt.title(' o$[n_R,n_T,T]$:[%s,%s,%s], lr:%s, size:%s, $\\mu_H$:%s' 
    ...
    plt.grid(True)
    # plt.show()
    plt.savefig('./simulation/result/olr%s_%s_ep%s_mu%s.png' %(lr, [2*n_R*T]+hidden_sizes+[2*2*n_R*n_T],num_epochs,H_mean))

    ## save the model
    torch.save(logits_net, './simulation/result/olr%s_%s_ep%s_mu%s.pt' %(lr, [2*n_R*T]+hidden_sizes+[2*2*n_R*n_T],num_epochs,H_mean))
