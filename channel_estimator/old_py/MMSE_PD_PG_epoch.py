...
def train(size, hidden_sizes=[64, 32], lr=1e-3, 
    ...
    with tqdm(total = num_epochs*num_minibatch) as pbar:
        ...
        for i in range(num_epochs):
            for j in range(num_minibatch):
                ...
                tau_h_hat = torch.zeros(num_trajectories, 2*n_R*n_T).to(device)
                tau_h = torch.zeros(num_trajectories, 2*n_R*n_T).to(device)
                tau_y = torch.zeros(num_trajectories, 2*n_R*n_T).to(device)
    ...
    plt.plot(iterations, torch.Tensor(iter_loss).to("cpu"), label='training loss')
    plt.plot(iterations, torch.Tensor(testing_loss).to("cpu"), label='validation loss')
