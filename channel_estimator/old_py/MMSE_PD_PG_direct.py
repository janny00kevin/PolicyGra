...
def train(size, hidden_sizes=[64, 32], lr=1e-3, channel_information = [0, 1, 0, 0.1],
    ...
    logits_net = mlp(sizes=[2*n_R*T]+hidden_sizes+[2*n_R*n_T]).to(device) ###
    ...
    with tqdm(total = num_epochs*num_minibatch) as pbar:
        ...
        for i in range(num_epochs):
            for j in range(num_minibatch):
                ...
                tau_h_hat = logits # Normal(logits[:,::2], logits[:,1::2]).sample() ###
                # if itr == 3:
                #     print(tau_y.size())
                #     print(tau_h.size())
                #     print(logits.size())
                ...
                batch_loss = lamb*(norm.unsqueeze(1)*logits_net(tau_y).log_prob(tau_h_hat)).mean() ###
                batch_loss.backward()
                optimizer.step()
                ## t update
                t -= lr/itr*(1-lamb)
                ...
                h_hat_test = logits #Normal(logits[:,::2], logits[:,1::2]).sample() ###
    ...
    ## save the model
    torch.save(logits_net, './simulation_dire/result/lr%s_%s_ep%s.pt' %(lr, [2*n_R*T]+hidden_sizes+[2*2*n_R*n_T],num_epochs))
